...
当用户使用中文、标的是 A 股/港股/中资公司、或输出进入中文 Obsidian Vault 时，Task 4 生成的图表必须中文可读：

- 图表标题、坐标轴、图例、数据标签、脚注、资料来源和图表索引均使用中文；不要输出 `Revenue by Product`、`DCF Sensitivity Analysis`、`Valuation Football Field` 这类英文标题。
- 图表字体使用可显示中文的字体，例如 PingFang SC、Microsoft YaHei、Source Han Sans、SimHei、Noto Sans CJK。生成后必须检查中文不乱码。
- 单位写在标题或轴标签里，例如 `收入（亿元）`、`毛利率（%）`、`目标价（元/股）`、`总市值（亿元）`。
- 图表命名可保留英文文件名以便程序处理，但图表内部文字和图表索引必须中文。
- 图表色彩和注释服务于阅读，不要用大段英文说明替代图表；关键结论可用 1 句中文短注释标在图下。
...
- chart_33: 目标价情景分析 → Valuation Summary tab (or calculate from scenarios)
...
def create_revenue_by_product_chart():
    ...
    # Source line
    fig.text(0.12, 0.02, '资料来源：公司公告、市场数据、[机构]估算',
...
def create_chart_index():
    ...
    optional_charts = [
        ...
        "chart_35_analyst_price_targets.png - 分析师目标价分布 [OPTIONAL]",
    ]

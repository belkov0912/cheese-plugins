...
当用户使用中文、标的是 A 股/港股/中资公司、或输出进入中文 Obsidian Vault 时，交付前必须完成以下检查：

- [ ] Markdown、DOCX、Excel、图表中的可见标题和正文以中文为主
- [ ] 首页使用 `首次覆盖`、`核心观点`、`投资逻辑`、`盈利预测与估值`、`风险提示` 等中文研报表达
- [ ] 评级使用 `买入/增持/中性/减持/卖出`；如保留英文评级，中文必须在前
- [ ] 目标价和股价使用本地交易币种，例如 A 股为 `元/股`
- [ ] 财务和估值单位使用 `亿元`、`百万元`、`万股`、`%` 等中文金融写法
- [ ] 表格列名、图表标题、坐标轴、图例、注释和资料来源为中文
- [ ] 图表中文字体不乱码，优先使用 PingFang SC、Microsoft YaHei、Source Han Sans、SimSun 或等价 CJK 字体
- [ ] 段落结论先行，避免公告摘抄式流水账；核心分歧区分 `事实 / 判断 / 假设 / 证伪点`
- [ ] 已搜索并修复英文模板残留：`COMPANY RESEARCH`、`Investment View`、`Price Target`、`Upside`、`BUY/HOLD/SELL`、`Mkt Cap`、`$M`、`Source:`
- [ ] 专有名词、股票代码、英文公司名、必要英文缩写可以保留，但不能让报告主体看起来像英文模板
...
- [ ] `首次覆盖` header present
- [ ] 中文投资主线标题，而不是泛泛事件标题
- [ ] 评级框包含评级、当前价格、目标价、52 周价格区间、总市值、企业价值
...
- [ ] Every figure has caption above: `图 X - [公司] [描述性标题]`
- [ ] Every figure has source line below: `资料来源：[具体来源和日期]`
- [ ] Sequential figure numbering (`图 1`, `图 2`, `图 3`... no gaps)

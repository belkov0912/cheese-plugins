...
name: r0-data
description: >-
  stock-selection-rules 的本地行情仓管理——全量下载（backfill，首次建仓约45-90分钟，断点续跑）、
  ...
  或 r0-breakout / 后续规则扫描报「本地仓为空/数据过旧」时，或输入 /r0-data。
  Do NOT use for: 查 R0 分数（用 r0-breakout）；非 A 股数据。
---

# R0 数据仓 · 全量下载 / 增量更新

stock-selection-rules 的各规则扫描与综合评级都读取本地行情仓（全A股日K parquet，后复权 hfq）。本 skill 负责建仓和保鲜。

## 脚本与数据在哪

- 脚本：本插件根目录 `scripts/store.py`（从本 SKILL.md 上两级即插件根；也可用 `${CLAUDE_PLUGIN_ROOT}/scripts`）。
- 数据目录：`$R0_DATA_DIR`，缺省 `~/.r0-data`。约 5300 个 parquet + `universe.csv`（票名快照），400 天日K合计约几百 MB。新股/次新股即使不足 50 根 K 线也会入仓，但 R0 打分会提示数据不足。
...
python3 - <<'PY'
import os, tempfile
import akshare, pandas as pd, yaml
...
- 通过 → 继续。
- 报 ModuleNotFoundError 或 parquet 读写失败 → 告诉用户缺哪个包，**征得同意后**帮装：`python3 -m pip install -r <插件根>/scripts/requirements.txt`（或单装缺的那个；parquet 失败通常是 `python3 -m pip install pyarrow`）。装完重跑自检确认。
- 如果当前 `python3` 所在的 conda 环境无法解析依赖，可先试 `/usr/bin/python3` 跑同一段自检；哪个 Python 通过，后续命令里的 `python3` 就替换成哪个。
...
6. **新上市的票由 backfill 纳入**（它拉最新票池）；不足 50 根 K 线的票会保存为短历史票，`stat` 会单独提示，R0 暂不打分。update 只保鲜已有票；隔一阵重跑一次 backfill 即可补新股（已覆盖到目标截止日的会跳过，不会重下）。
...
- `universe.csv` 是建仓时顺手存的票名快照，r0-breakout 等查询入口用它做「名称→代码」解析；新股上市后要重跑一次 backfill 才能按名字查（按代码查不受影响）。

"""Backtest the R7 probe-and-pullback rule on the local A-share kline store.

This script compares:
- R7: the relative-volume probe -> pullback definition in r7.py
- R7_absolute_baseline: the absolute-turnover baseline in score.py

Backtest protocol:
- For each stock, pick historical anchors every outcome_len trading days.
- At each anchor, truncate the frame to <= anchor before scoring R7.
...
def _write_report(windows, pool, args, out_dir, cfg):
    ...
    rules = [("R7", "R7"), ("绝对换手基线", "R7_absolute_baseline")]
    summaries = bt.summary_rows(windows, rules)

    report = []
    report.append(f"# R7 历史回测 {run_date}\n")
    report.append("## 口径\n")
    ...
    report.append("- R7 每个锚点都先截断到当日再算，避免未来K线进特征。")
    report.append("- 绝对换手基线用于检验相对放量规则的增量。\n")
    ...
    report.append("\n## 快速结论\n")
    current = next(x for x in summaries if x["rule"] == "R7")
    baseline = next(x for x in summaries if x["rule"] == "绝对换手基线")
    report.append(f"- R7 均值 IC：{bt.fmt_num(current['mean_ic'], 2)}，IC>0 窗口：{current['ic_positive']}/{current['ic_windows']}。")
    report.append(f"- 绝对换手基线均值 IC：{bt.fmt_num(baseline['mean_ic'], 2)}，IC>0 窗口：{baseline['ic_positive']}/{baseline['ic_windows']}。")
...
def run(args):
    ...
    for i, sym in enumerate(syms):
        ...
        while True:
            ...
            current_score, current_desc, _ = r7.score_r7(hist)
            if current_score is None:
                k += 1
                continue
            baseline_feat = features.compute_features(frame, anchor, cfg)
            if baseline_feat is None:
                k += 1
                continue
            baseline_score, baseline_desc = score.score_R7(baseline_feat, cfg)
            row = {
                "code": sym,
                "anchor_date": frame["date"][anchor],
                "data_last": frame["date"][-1],
                "R7": int(current_score),
                "R7_absolute_baseline": int(baseline_score),
                "outcome_ret": outcome,
                "R7_desc": current_desc,
                "R7_absolute_baseline_desc": baseline_desc,
            }

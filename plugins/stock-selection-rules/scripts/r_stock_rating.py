...
The script intentionally reuses the stock-selection-rules plugin implementation:
...
if not RULE_SCRIPTS.exists():
    raise SystemExit(f"stock-selection-rules scripts not found: {RULE_SCRIPTS}")
...
def latest_complete_trade_date() -> str:
    return store._data_end_date()  # intentionally uses stock-selection-rules's Beijing-time logic
...
def write_report(pool: pd.DataFrame, meta: dict[str, str], args: argparse.Namespace) -> Path:
    ...
    lines.append("- 申万历史行业分类按 start_date 还原，但下载文件可能含事后修订；未保存逐日快照的回测不能排除修订偏差。")
...
def run_latest(args: argparse.Namespace) -> None:
    ...
    # (实测坍缩成 S=1/A=1 且零警告)。按覆盖率把关:追平 data_max 的票占比过低就拒绝。
    ...
    if R1_REFERENCE_HAS_REVISION_RISK:
        print("# 注意：R1 使用申万动态行业主线；历史行业分类可能含事后修订。不下单、不荐股。")
    ...
    for r in rows:
        ...
        # #12: 单票模式带上 R7 证据化描述(试盘日/量比/回踩缩量比),对齐 r7-reset 的信息量。
        if single:

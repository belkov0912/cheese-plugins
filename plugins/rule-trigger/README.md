...
规则触发器工具箱：提供 R0「启动扳机」、R7「试盘后回踩」、R9「下跌反转」、R1R3「主线内强度」即时查和本地行情仓，后续可扩展其余规则评分、扫描和复盘。R0 查最近 5 个交易日里有没有「放量强阳突破」；R7 查最近 20 个交易日里有没有「相对放量试盘 → 缩量回踩」；R9 查「刚跌完企稳的票，最近 5 个交易日有没有放量大阳收复失地」；R1R3 查「在不在 AI 主线白名单、20 日涨幅在板块内排第几」。**自带全部脚本，行情数据下载到用户本地**——装上插件、建一次仓，任何机器都能用。

...
name: r0-data
description: >-
  rule-trigger 的本地行情仓管理——全量下载（backfill，首次建仓约45-90分钟，断点续跑）、
  ...
  或 r0-trigger / 后续规则扫描报「本地仓为空/数据过旧」时，或输入 /r0-data。
  Do NOT use for: 查 R0 分数（用 r0-trigger）；非 A 股数据。
---

# R0 数据仓 · 全量下载 / 增量更新

r0-trigger 以及后续 R1-R8 规则扫描要读本地行情仓（全A股日K parquet，后复权 hfq）。本 skill 负责建仓和保鲜。
...
- 如果当前 `python3` 指向旧 conda 环境导致依赖解不开，可先试 `/usr/bin/python3` 跑同一段自检；哪个 Python 通过，后续命令里的 `python3` 就替换成哪个。
...
- `universe.csv` 是建仓时顺手存的票名快照，r0-trigger 等查询入口用它做「名称→代码」解析；新股上市后要重跑一次 backfill 才能按名字查（按代码查不受影响）。

...
当用户使用中文、标的是 A 股/港股/中资公司、或输出进入中文 Obsidian Vault 时，本模板只作为结构参考，所有可见文本必须改成中文研报表达：

- 首页标题用 `首次覆盖`，不要用裸英文 `INVESTMENT UPDATE`、`COMPANY UPDATE` 或 `INITIATING COVERAGE`。
- 评级框字段用中文：`评级`、`当前价格`、`目标价`、`52 周价格区间`、`总市值`、`企业价值`、`上涨空间`。
- 图表编号用 `图 1 - ...`，表格编号用 `表 1 - ...`，资料来源写 `资料来源：...`。
- A 股默认单位为 `元/股`、`亿元`、`百万元`、`%`；不要保留 `$M`、`$B`、`Market Cap`、`Enterprise Value` 等英文列名。
- 中文 DOCX 使用可读 CJK 字体，例如 PingFang SC、Microsoft YaHei、Source Han Sans、SimSun。
- 英文示例仅用于理解布局，不得原样复制到中文交付物。
...
**CRITICAL**: Page 1 is NOT a traditional executive summary. For Chinese deliverables, it should be a compact sell-side research front page with clear `核心观点`、`投资逻辑`、`盈利预测与估值`、`催化剂` and `风险提示`.
...
Revenue ($M)                [X]         [X]         [X]         [X]        [X]
收入增速（%）              X.X%        X.X%        X.X%        X.X%       X.X%
毛利率（%）                X.X%        X.X%        X.X%        X.X%       X.X%
EBITDA（百万元）           [X]         [X]         [X]         [X]        [X]
EBITDA率（%）              X.X%        X.X%        X.X%        X.X%       X.X%
EPS（元/股）               X.XX        X.XX        X.XX        X.XX       X.XX
...
1. **Sequential numbering**: 图 1、图 2、图 3 等，全文连续编号
2. **Descriptive title**: `图 X - [公司] [具体指标] [图表类型/分析主题]`
...
图 X - [描述性标题]
[Table content]
资料来源：公司公告、FactSet/Wind/Choice、[机构名]估算。
```
...
```
估值方法                    权重      隐含价值        加权价值
DCF 估值                    50%       XX-YY 元/股     ZZ 元/股
可比公司估值                30%       XX-YY 元/股     ZZ 元/股
可比交易估值                20%       XX-YY 元/股     ZZ 元/股
                           ────       ─────────────    ─────────
加权平均目标价                        AA-BB 元/股     CC 元/股
```
...
```
情景          企业价值            股权价值          目标价
悲观          XX 亿元             XX 亿元           XX 元/股
中性          XX 亿元             XX 亿元           XX 元/股
乐观          XX 亿元             XX 亿元           XX 元/股
```
...
```
公司            代码     总市值    EV/收入  EV/收入  EV/EBITDA  EV/EBITDA  收入增速 EBITDA率
                       （亿元）   2024E   2025E   2024E      2025E
...
资料来源：FactSet/Wind/Choice、公司公告、[机构名]估算。
```

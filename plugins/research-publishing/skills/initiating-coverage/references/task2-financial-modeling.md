...
当用户使用中文、标的是 A 股/港股/中资公司、或输出进入中文 Obsidian Vault 时，Task 2 必须生成中文可读的财务模型：

- Sheet 名、表格标题、行标签、注释和假设说明优先使用中文。允许在关键表名后保留英文缩写，例如 `DCF输入`、`EV/EBITDA`、`P/E`。
- A 股默认人民币口径，单位使用 `百万元`或`亿元`，每张表顶部必须写清楚单位。不要使用 `$M`、`$B`、`Mkt Cap` 等英文/美元默认口径，除非公司本身以美元披露。
- 推荐核心表名：`收入模型`、`利润表`、`现金流量表`、`资产负债表`、`情景假设`、`DCF输入`。如果需要兼容英文，可写成 `收入模型 Revenue Model`，但中文在前。
- 三种情景默认翻译为 `乐观`、`中性`、`悲观`，可在括号中保留 `Bull/Base/Bear`。
- 对 A 股/港股公司，财务数据优先来自公司年报、季报、交易所公告、巨潮资讯、港交所披露易、公司 IR 材料和 Wind/Choice/同花顺等数据源；不要把 SEC 10-K 作为唯一默认前提。
- 假设说明必须用中文解释“为什么这样预测”，而不是只给增长率。关键假设至少说明历史依据、管理层指引、行业驱动、敏感性风险。
...
- **Required**: Access to company financial data
  - For US public companies: Latest 10-K and recent 10-Qs from SEC EDGAR
  - For A-share/H-share/HK companies: latest annual report, quarterly reports, exchange announcements, CNINFO/SSE/SZSE/HKEX filings, or company IR materials
...
- Clear units (`百万元`, `亿元`, `元/股`, or disclosure currency)
- Chinese deliverables must use Chinese row labels, Chinese notes, and Chinese table titles
...
1. **Download annual report or equivalent filing**
   ...
   - For A-share/H-share/HK companies, use CNINFO, SSE, SZSE, HKEXnews, exchange announcements, or company IR PDFs instead of SEC EDGAR

...
本文件是方法论参考，可以保留 DCF、WACC、EV/EBITDA、P/E 等必要英文缩写。但当用户使用中文、标的是 A 股/港股/中资公司、或输出进入中文 Obsidian Vault 时，最终交付物必须按中文估值报告表达：

- 章节标题使用 `DCF 估值`、`可比公司估值`、`可比交易估值`、`估值汇总`、`目标价与投资建议`。
- 表格字段使用中文单位和中文列名，例如 `企业价值（亿元）`、`股权价值（亿元）`、`目标价（元/股）`、`上涨空间`。
- 公式可以保留英文变量名，但解释文字必须中文，且必须说明假设来源、适用性和证伪点。
- A 股估值默认使用人民币、`元/股`、`亿元`；无风险利率优先用中国 10 年期国债收益率。
